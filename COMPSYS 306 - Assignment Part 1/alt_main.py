...
datadir = 'Combined-Dataset/myData'
pickle_filepath = "pickled.pickle"
i=0
...
flat_data_arr = []
target_arr = []
model_filename = 'SVCModel_alt.joblib'
...
if os.path.exists(pickle_filepath):
    ...
    for category in categories:
        ...
        # Resize the image to size of 32x32 pixels with 3 slots for the RGB values
            img_resized=resize(img_array,(66,66,3))
...
# param_grid={'C':[0.1,1,10],'gamma':[0.0001,0.001,0.1,1],'kernel':['rbf','poly'],'degree':[2,3,4]}
...
svcModel_filename = 'SVCModel_big_alt.joblib'
if os.path.exists(svcModel_filename):
    print("Found Existing SVC")
    svcModel = joblib.load(svcModel_filename)
else:
    print("Training SVC")
    svcModel = svm.SVC(kernel='poly',gamma=0.0001,C=0.1,max_iter=1000,probability=False,degree=2)
...
for image in images:
    ...
    image = resize(imread(image),(112,112,3)).reshape(1,-1)
    # image = StandardScaler().fit_transform(image)
    svcPred = svcModel.predict(image)
    end_time = time.time()
